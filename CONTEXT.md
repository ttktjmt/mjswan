...
Two more directories under `src/core/` are viewer-only and no part of any MDP: `interaction/` (the pointer's modes — pull, push, grab and throw — behind one `InteractionManager`, which is also the single writer of `xfrc_applied`, plus the box pool a scene compiles in) and `grab/` (`weldHold.ts`, the constraint the pointer and the tracked hands both hold things with; what triggers a grab stays with each). Neither reaches Python, the manifest or the `.swn` document.
