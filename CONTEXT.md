...
Two more directories under `src/core/` are viewer-only and no part of any MDP: `interaction/` (the pointer's view, pull, push and grab modes behind one `InteractionManager`, which is also the single writer of `xfrc_applied`) and `grab/` (`weldHold.ts`, the constraint the pointer and the tracked hands both hold things with; what triggers a grab stays with each). Neither reaches Python, the manifest or the `.swn` document.
