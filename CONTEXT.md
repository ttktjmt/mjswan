...
- Runs the policy **and every traced MDP term body** via onnxruntime-web — the policy on WebGPU where the browser has it (ORT falls back to wasm on its own), term graphs always on wasm since each is small and a step runs many.
...
- **Library** (`vite.lib.config.ts`, `npm run build:lib`) — a single self-contained ESM `dist/mjswan.js` exposing `createEngine(element, options?)` (entry `src/engine/index.ts`), consumed by mjswan Cloud from a CDN. Every dependency is bundled (no bare imports) and the MuJoCo/ONNX WASM is emitted as co-located files referenced via `new URL('./x.wasm', import.meta.url)`. Vite lib mode force-inlines those WASM as base64; a `generateBundle` plugin extracts them back into co-located files. See mjswan-cloud ADR 0001. Each extracted file is recognized by content digest so it keeps its upstream basename, and `src/core/onnx/ortEnv.ts` points `ort.env.wasm.wasmPaths` at ORT's — so a host serving `dist/` fetches nothing third-party (issue #123). Only `mjswan.js` sits at the root of `dist/`, where it is addressed from outside as the npm entry; everything the build generates goes into `dist/assets/` alongside the SPA build's output, which names files the same way, so identical bytes land on one path instead of shipping 40 MiB twice. `vite.wasm.ts` carries the sources and the ORT assumptions this rests on.
