...
- **License files travel with the build**
  ...
  `NOTICE` for the work — `Builder(license=, copyright=)`, `add_project(license=…)`,
  `ProjectHandle.set_license()` / `set_notice()`, an SPDX id generating the standard text
  with its tag or a path copied verbatim — and `<project-id>/<scene-id>/LICENSE.<component>`
  / `NOTICE.<component>` for each third-party component. `add_scene(spec=…)` detects the
  latter beside the model and the directories its meshes resolve to (two parents at most,
  nearest level wins, verbatim), `add_scene_mjlab` falls back to a known-assets table by
  task id, and `SceneHandle.add_attribution()` / `clear_attributions()` declare what no
  file can be found for (motion clips, weights). `mjswan publish` admits the names beside
  the extension allowlist as `text/plain`, prints each file with its identified license,
  warns for a restricted one (CC NC / SA / ND, GPL family) and refuses one that forbids
  redistribution before any network call; `mjswan info` lists them. `manifest.json` and
  `format` are unchanged; the root `dist/LICENSE` stays the engine's and is never
  published. `examples/demo/main.py::_copy_licenses` is retired.
