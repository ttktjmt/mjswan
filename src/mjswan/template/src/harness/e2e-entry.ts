 ...
 * `?format=mjb` for a compiled model, `?loads=` times), lets the physics + render loops
 * run, then reads back `captureThumbnail()` and checks the frame is non-blank. Result is
 * published on `window.__harness` for the test.
 */
...
async function main(): Promise<void> {
  ...
  // `?loads=2` switches scenes once, freeing the first model while the render loop runs.
  const loads = Number(params.get('loads') ?? '1');
  for (let i = 0; i < loads; i++) await engine.loadScene({ model, modelFormat });
