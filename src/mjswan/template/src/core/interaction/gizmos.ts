...
/**
 * The box about to be thrown, drawn where it will appear. Not a physics object — it exists
 * so the aim has something to aim, and it is rebuilt when the size slider moves.
 */
export class SpawnGhost {
  ...
  /** `size` is the half-extent, as MuJoCo states it. */
  show(at: THREE.Vector3, size: number): void {
    if (size !== this.drawnSize) {
      this.drawnSize = size;
      this.mesh.geometry.dispose();
      this.mesh.geometry = new THREE.BoxGeometry(size * 2, size * 2, size * 2);
    }
    this.mesh.position.copy(at);
    this.mesh.visible = true;
  }
